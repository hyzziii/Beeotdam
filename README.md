...
셋 다 PR과 `main`·`develop` 푸시마다 GitHub Actions에서 돌아갑니다
...
│  └─ settings.tsx         설정
│
├─ api/                    공공데이터포털 호출 (*.test.ts는 그 옆에 둔다)
...
화면이 아니라 **값을 넣으면 값이 나오는 함수**만 테스트합니다. 앱을 켜거나 API를 부를
필요가 없어 1초 남짓이면 32개가 끝납니다.

| 대상 | 무엇을 지키나 |
|---|---|
| `api/kma-time.ts` | 발표 시각 경계. 14:10에는 11시 발표를, 01:00에는 전날 23시 발표를 |
| `api/grid.ts` | 위경도 → 격자. 서울 `(60, 127)`과 격자 범위·방향 |
| `api/kma.ts` | 예보 끝에 붙는 하루 미만 조각을 버리는 규칙 |
| `lib/sun.ts` | 일출·일몰이 발표값과 3분 안에 들어오는지 |
| `lib/dam.ts` | 홍수 경보 경계. 기준에 '닿았을 때'부터 경보 |

`api/kma.ts` 테스트는 실제로 났던 버그에서 나왔습니다. 주간 날씨의 금요일이 기온 없이
`—`로 떴던 일인데, 고친 코드를 일부러 되돌려 그 테스트만 실패하는 것을 확인했습니다.
실패하지 않는 테스트는 지켜주는 게 없습니다.

격자 테스트는 도시별 좌표를 여럿 박아 두려다 그만두었습니다. 기상청 지점 좌표표를 직접
확인하지 않고 기억으로 적었더니 틀린 기대값이 맞는 코드를 실패시켰습니다. 확실한 짝
하나만 고정하고, 나머지는 좌표를 몰라도 참인 성질로 검증합니다.
...
- [x] 테스트 · CI — 순수 함수 32개. lint·tsc·test를 GitHub Actions에서 돌린다
