...
> **개발 단계 안내**
> 홈의 현재 날씨·우산 알림·시간대별 강수는 **기상청 실제 데이터**로 동작합니다.
> 주간 날씨, 날씨 탭의 대기 환경, 수자원 탭의 댐 수치는 아직 `src/data/`의 샘플입니다.
>
...
│  └─ kwater.ts            댐 수문 정보 (댐 코드 확인 후 연결 예정)
│
├─ components/
...
│
├─ weather/
│  ├─ weather-context.tsx  날씨 조회 + 캐시 (지역이 바뀌면 다시 불러옴)
│  └─ failure-message.ts   실패 종류별 안내 문구
│
├─ hooks/
...
지금까지 날씨가 그렇게 교체됐고, 댐 데이터가 남아 있습니다.
...
- [x] 설정값 영구 저장
- [x] 전국 시/군/구 지역 선택
- [x] 기상청 연동 — 현재 날씨, 우산 알림, 시간대별 강수
- [ ] 주간 날씨 — 중기예보 연동 (단기예보는 4일까지만 준다)
- [ ] 날씨 탭 — 시간별 목록, 대기 환경 (에어코리아 별도 신청 필요)
- [ ] K-water 연동 — 댐 코드를 '수문 제원 현황'에서 받아오는 것부터
- [ ] 로딩 · 오류 화면
